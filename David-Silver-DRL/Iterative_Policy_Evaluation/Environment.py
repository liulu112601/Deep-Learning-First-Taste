    ...
    def update_value_net(self,value_net):
        ...
        for line in range(self.total_lines):
            for col in range(self.total_cols):
                ...
                new_value_net[line,col] = (1/self.total_actions)*(self.imm_reward + self.discount_factor*self.value_net[left[0],left[1]])+ \
                                          (1 / self.total_actions) * (self.imm_reward + self.discount_factor*self.value_net[up[0], up[1]])+\
                                          (1 / self.total_actions) * (self.imm_reward + self.discount_factor*self.value_net[right[0], right[1]])+\
                                          (1 / self.total_actions) * (self.imm_reward + self.discount_factor*self.value_net[down[0], down[1]])
        #start and terminal are zero
        new_value_net[0,0] = 0
        new_value_net[self.total_lines-1,self.total_cols-1] = 0

        return new_value_net

    def get_value(self,line,col):
        return self.value_net[line,col]
